...
export default function Cart({ cart, user, updateQuantity, removeFromCart, setActiveTab, clearCart }) {
  ...
  const handleApplyCoupon = () => {
    // Demo logic - ideally this calls the backend
  ...
  const handleCheckout = async () => {
    ...
    try {
      const payload = {
        ...
        shipping_address: "Địa chỉ mặc định của người dùng", // Ideal: prompt for address
  ...
  const styles = {
    ...
    emptyState: { textAlign: "center", padding: "100px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: "20px" }
  };
    ...
    return (
      <div style={styles.container}>
        <div style={styles.emptyState}>
          ...
          <button onClick={() => setActiveTab("shop")} style={{ padding: "16px 40px", backgroundColor: brand.secondary, color: "white", border: "none", borderRadius: "12px", fontWeight: "700", cursor: "pointer" }}>MUA SẮM NGAY</button>
        </div>
      </div>
    );
  }
      ...
      <div style={styles.wrapper}>
        <div style={styles.cartSection}>
          ...
          {cart.map((item) => (
            <div key={item.id} style={styles.item}>
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: "16px", fontWeight: "700", marginBottom: "8px", color: brand.primary, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: "1.2" }}>{item.name}</h3>
        ...
        <div style={styles.summarySection}>
          ...
          <div style={{ display: "flex", margin: "25px 0" }}>
            <input style={{ flex: 1, padding: "12px", border: `1px solid ${brand.border}`, borderRadius: "10px 0 0 10px", outline: "none", fontSize: "14px" }} placeholder="Nhập mã giảm giá" value={promo} onChange={(e) => setPromo(e.target.value)} />
            <button onClick={handleApplyCoupon} style={{ backgroundColor: brand.primary, color: "#fff", border: "none", padding: "0 20px", cursor: "pointer", fontWeight: "600", borderRadius: "0 10px 10px 0" }}>Áp dụng</button>
          </div>
          ...
          <div style={{ borderTop: `1px solid ${brand.border}`, paddingTop: "20px", marginBottom: "30px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              ...
              <div style={{ textAlign: "right" }}>
                {discount > 0 && <div style={{ fontSize: "14px", color: "#e74c3c", fontWeight: "700", marginBottom: "5px" }}>Giảm giá: -{formatVnd(discount)}</div>}
          ...
          <button 
            ...
            style={{ width: "100%", padding: "18px", backgroundColor: brand.primary, color: "white", border: "none", borderRadius: "15px", fontWeight: "800", fontSize: "16px", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", marginBottom: "15px" }}>
            {loading ? "ĐANG XỬ LÝ..." : "THANH TOÁN NGAY"} <ArrowRight size={20} />
          </button>
          <button onClick={() => setActiveTab("shop")} style={{ width: "100%", padding: "15px", backgroundColor: "transparent", color: brand.primary, border: `2px solid ${brand.primary}`, borderRadius: "15px", fontWeight: "700", cursor: "pointer" }}>Tiếp tục mua sắm</button>
        </div>
      </div>
    </div>
  );
